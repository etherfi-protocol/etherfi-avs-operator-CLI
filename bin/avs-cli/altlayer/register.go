...
var AltLayerRegisterCmd = &cli.Command{
	Name:   "register",
	Usage:  "(Admin) Register target operator to AltLayer AVS",
	Action: handleAltLayerRegister,
	Flags: []cli.Flag{
		&cli.StringFlag{
...
func handleAltLayerRegister(ctx context.Context, cli *cli.Command) error {

	// parse cli params
	inputFilepath := cli.String("registration-input")

	// read input file with required eigenDA data
	var input altlayer.RegistrationInfo
	...
	return altlayerAPI.RegisterOperator(operator, input, signingKey)
}
