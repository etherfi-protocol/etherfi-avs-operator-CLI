...
import (
	...
	"github.com/etherfi-protocol/etherfi-avs-operator-tool/src/avs/redstone"
...
var redstoneAPI *redstone.API
var etherfiAPI *etherfi.API

var RedstoneCmd = &cli.Command{
	Name:   "redstone",
	Usage:  "various actions related to managing Redstone operators",
	Before: prepareCmd,
...
func prepareCmd(ctx context.Context, cmd *cli.Command) error {
	...
	redstoneAPI = redstone.New(cfg, rpcClient)
...
func handleRegister(ctx context.Context, cli *cli.Command) error {
	...
	return redstoneAPI.RegisterOperator(operator, &othenticOutput, signingKey, whitelistEnabled)
}
...
func handleUnregister(ctx context.Context, cli *cli.Command) error {
	...
	return redstoneAPI.UnregisterOperator(operator)
}
