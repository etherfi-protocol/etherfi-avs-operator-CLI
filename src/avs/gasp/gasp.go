...
// API handle for all core Gasp functionality
type API struct {
...
func New(cfg config.Config, rpcClient *ethclient.Client) *API {
	...
	registryCoordinator, _ := NewRegistryCoordinator(cfg.GaspRegistryCoordinatorAddress, rpcClient)

	return &API{
		Client:                     rpcClient,
		RegistryCoordinator:        registryCoordinator,
		RegistryCoordinatorAddress: cfg.GaspRegistryCoordinatorAddress,
		ServiceManagerAddress:      cfg.GaspServiceManagerAddress,
...
func (a *API) PrepareRegistration(operator *etherfi.Operator, blsKey *bls.KeyPair, socket string, quorums []int64) error {
	...
	return utils.ExportJSON("gasp-prepare-registration", operator.ID, ri)
}
...
func (a *API) RegisterOperator(operator *etherfi.Operator, info RegistrationInfo, signerKey *ecdsa.PrivateKey) error {
	...
	batch := gnosis.NewSingleTxBatch(adminCall, a.AvsOperatorManagerAddress, fmt.Sprintf("gasp-register-operator-%d", operator.ID))
	return utils.ExportJSON("gasp-register-gnosis", operator.ID, batch)
}
