...
// API handle for all core CyberMACH functionality
type API struct {
...
func New(cfg config.Config, rpcClient *ethclient.Client) *API {
	...
	registryCoordinator, _ := NewRegistryCoordinator(cfg.CyberMachRegistryCoordinatorAddress, rpcClient)

	return &API{
		Client:                     rpcClient,
		RegistryCoordinator:        registryCoordinator,
		RegistryCoordinatorAddress: cfg.CyberMachRegistryCoordinatorAddress,
		ServiceManagerAddress:      cfg.CyberMachServiceManagerAddress,
...
func (a *API) PrepareRegistration(operator *etherfi.Operator, blsKey *bls.KeyPair, socket string, quorums []int64) error {
	...
	return utils.ExportJSON("cybermach-prepare-registration", operator.ID, ri)
}
...
func (a *API) RegisterOperator(operator *etherfi.Operator, info RegistrationInfo, signerKey *ecdsa.PrivateKey) error {
	...
	batch := gnosis.NewSingleTxBatch(adminCall, a.AvsOperatorManagerAddress, fmt.Sprintf("cybermach-register-operator-%d", operator.ID))
	return utils.ExportJSON("cybermach-register-gnosis", operator.ID, batch)
}
