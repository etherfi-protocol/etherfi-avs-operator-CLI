...
// API handle for all core Redstone functionality
type API struct {
...
func New(cfg config.Config, rpcClient *ethclient.Client) *API {
	...
	avsGovernance, _ := NewAVSGovernance(cfg.RedstoneAVSGovernanceAddress, rpcClient)

	return &API{
		Client:                    rpcClient,
		AVSGovernance:             avsGovernance,
		AVSGovernanceAddress:      cfg.RedstoneAVSGovernanceAddress,
		ServiceManagerAddress:     cfg.RedstoneAVSGovernanceAddress, // AVSGovernance contract serves as the service manager for this AVS
...
func (a *API) RegisterOperator(operator *etherfi.Operator, othenticOperatorData *othentic.CLIOperatorData, signerKey *ecdsa.PrivateKey, whitelistEnabled bool) error {
	...
	batch := gnosis.NewSingleTxBatch(adminCall, a.AvsOperatorManagerAddress, fmt.Sprintf("redstone-register-operator-%d", operator.ID))
	return utils.ExportJSON("redstone-register-gnosis", operator.ID, batch)
}
...
func (a *API) UnregisterOperator(operator *etherfi.Operator) error {
	...
	// output in gnosis compatible format
	batch := gnosis.NewSingleTxBatch(adminCall, a.AvsOperatorManagerAddress, fmt.Sprintf("redstone-unregister-operator-%d", operator.ID))
	return utils.ExportJSON("redstone-unregister-gnosis", operator.ID, batch)
}
...
// Redstone is an Othentic based AVS
func (a *API) PrepareRegistration(operator *etherfi.Operator, operatorData *othentic.CLIOperatorData) error {
	...
	return utils.ExportJSON("redstone-prepare-registration", operator.ID, ri)
}
*/
