...
// API handle for all core hyperlane functionality
type API struct {
...
func New(cfg config.Config, rpcClient *ethclient.Client) *API {
	...
	stakeRegistry, _ := NewECDSAStakeRegistry(cfg.HyperlaneStakeRegistryAddress, rpcClient)

	return &API{
		Client: rpcClient,

		ECDSAStakeRegistry:        stakeRegistry,
		ECDSAStakeRegistryAddress: cfg.HyperlaneStakeRegistryAddress,
...
func (a *API) PrepareRegistration(operator *etherfi.Operator, avsSigner common.Address) error {
	...
	return utils.ExportJSON("hyperlane-prepare-registration", operator.ID, ri)
}
...
func (a *API) RegisterOperator(operator *etherfi.Operator, info RegistrationInfo, signingKey *ecdsa.PrivateKey) error {
	...
	batch := gnosis.NewSingleTxBatch(adminCall, a.AvsOperatorManagerAddress, fmt.Sprintf("hyperlane-register-%d", operator.ID))
	return utils.ExportJSON("hyperlane-register-gnosis", operator.ID, batch)
}
