...
func New(cfg config.Config, rpcClient *ethclient.Client) *API {
	...
	registryCoordinator, _ := NewRegistryCoordinator(cfg.AutomataRegistryCoordinatorAddress, rpcClient)
...
func (a *API) PrepareRegistration(operator *etherfi.Operator, blsKey *bls.KeyPair, socket string, quorums []int64) error {
	...
	return utils.ExportJSON("automata-prepare-registration", operator.ID, ri)
}
...
func (a *API) RegisterOperator(operator *etherfi.Operator, info RegistrationInfo, signerKey *ecdsa.PrivateKey) error {
	...
	batch := gnosis.NewSingleTxBatch(adminCall, a.AvsOperatorManagerAddress, fmt.Sprintf("automata-register-operator-%d", operator.ID))
	return utils.ExportJSON("automata-register-gnosis", operator.ID, batch)
}
